...
void easyppm_draw_line(PPM *ppm, int x0, int y0, int x1, int y1, ppmcolor c) {
  ...
  if (x0 < 0 || x0 > ppm->width) easyppm_abort(ppm, "ERROR : invalid x1 value in easyppm_draw_line()");
  if (x1 < 0 || x1 > ppm->width) easyppm_abort(ppm, "ERROR : invalid x2 value in easyppm_draw_line()");
  if (y0 < 0 || y0 > ppm->height) easyppm_abort(ppm, "ERROR : invalid y1 value in easyppm_draw_line()");
  if (y1 < 0 || y1 > ppm->height) easyppm_abort(ppm, "ERROR : invalid y2 value in easyppm_draw_line()");
...
void easyppm_fill_polygon(PPM *ppm, int nbpoints, int *xcoordinates, int *ycoordinates, ppmcolor cin, ppmcolor cborder) {
  ...
  for (pixelY=0; pixelY < ppm->height; pixelY++) {
	...
	nodeX[nodes++]=(int) ceilf((double)polyX[i]+
				   (double)(pixelY-polyY[i])/(double)(polyY[j]-polyY[i])
				   *(double)(polyX[j]-polyX[i]));
      }
      j=i;
    }
